...
describe("SwapAdapter", function () {
    ...
    it("rescueFunds correct", async () => {
        ...
        await swapAdapter.connect(wallet).rescueFunds(ethers.constants.AddressZero,wallet.address,ethers.utils.parseEther('1'));
    ...
    it("rescueFunds only owner", async () => {
        let [wallet, other] = await ethers.getSigners();
        await deployFixture();
        await expect(swapAdapter.connect(other).rescueFunds(ethers.constants.AddressZero,wallet.address,100)).to.be.revertedWith("Ownable: caller is not the owner");
    })
