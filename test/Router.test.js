...
describe("Router", function () {
  ...
  it("swapAndCall", async () => {
    ...
    await (
      await router
        .connect(user)
        .swapAndCall(ethers.constants.HashZero, _srcToken, _amount, user.address, _swapData, _callData, _permitData, {
  ...
  it("swapAndCall", async () => {
    ...
    await expect(
      router
        .connect(user)
        .swapAndCall(ethers.constants.HashZero, _srcToken, _amount, user.address, _swapData, _payData, _permitData)
    )
      .to.be.emit(pay, "Pay")
