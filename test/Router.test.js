...
describe("Router", function () {
    ...
    it("setAuthorization only owner", async () => {
        let [wallet, other] = await ethers.getSigners();
        await deployFixture();
        await expect(router.connect(other).setAuthorization([swapAdapter.address], true)).to.be.revertedWith("Ownable: caller is not the owner");
    })
    ...
    it("setFeeManager not contract", async () => {
        let [wallet, other] = await ethers.getSigners();
        await deployFixture();
        await expect(router.connect(wallet).setFeeManager(ethers.constants.AddressZero)).to.be.revertedWith("Router: not contract");
    })
    ...
    it("setAuthorization only owner", async () => {
        let [wallet, other] = await ethers.getSigners();
        await deployFixture();
        await expect(router.connect(wallet).setAuthorization([wallet.address], true)).to.be.revertedWith("Router: not contract");
    })

    it("setAuthorization correct", async () => {
        let [wallet, other] = await ethers.getSigners();
        await deployFixture();
        await expect(router.connect(wallet).setAuthorization([swapAdapter.address], true)).to.be.emit(router, "Approve");
        let p = await router.approved(swapAdapter.address);
        expect(p).to.be.true;
        await expect(router.connect(wallet).setAuthorization([swapAdapter.address], false)).to.be.emit(router, "Approve");
...
//  call rubic
 it("swapAndCall", async () => {
    ...
    await(await router.connect(user).swapAndCall(ethers.constants.HashZero,_srcToken,_amount,user.address,_swapData,_callData,_permitData,{value:extraNativeAmount})).wait();
...
it("swapAndCall", async () => {
    ...
    await(await token.approve(router.address,_amount)).wait();  
    await(await router.connect(user).swapAndCall(ethers.constants.HashZero,_srcToken,_amount,user.address,_swapData,_callData,_permitData)).wait();
...
it("swapAndCall", async () => {
    ...
    let token = await ethers.getContractAt(ERC20, _srcToken, user); 
    await(await token.approve(router.address,_amount)).wait();  
    await expect(router.connect(user).swapAndCall(ethers.constants.HashZero,_srcToken,_amount,user.address,_swapData,_payData,_permitData)).to.be.emit(pay,"Pay").emit(router,"SwapAndCall");
