...
describe("FeeManager", function () {
    ...
    it("initialFeeStruct", async () => {
        let [wallet, other] = await ethers.getSigners();
        await deployFixture();
        await expect(feeManager.connect(other).initialFeeStruct(1,1000,1000)).to.be.revertedWith("Ownable: caller is not the owner")
        await expect(feeManager.connect(wallet).initialFeeStruct(2,1000,10000)).to.be.revertedWith("invalid platformTokenFee")
    ...
    it("setIntegratorFees", async () => {
        ...
        await expect(feeManager.connect(other).setIntegratorFees(wallet.address,feeInfo)).to.be.revertedWith("Ownable: caller is not the owner")
        await expect(feeManager.connect(wallet).setIntegratorFees(ethers.constants.AddressZero,feeInfo)).to.be.revertedWith("Router: zero addr")
        feeInfo.feeType = 2;
        feeInfo.tokenFee = 10000;
        await expect(feeManager.connect(wallet).setIntegratorFees(wallet.address,feeInfo)).to.be.revertedWith("invalid tokenFee")
        feeInfo.feeType = 2;
        feeInfo.tokenFee = 100;
        feeInfo.platformTokenShare = 10000;
        await expect(feeManager.connect(wallet).setIntegratorFees(wallet.address,feeInfo)).to.be.revertedWith("invalid platformTokenShare")
        feeInfo.platformTokenShare = 100;
        feeInfo.platformNativeShare = 10000
        await expect(feeManager.connect(wallet).setIntegratorFees(wallet.address,feeInfo)).to.be.revertedWith("invalid platformNativeShare")
        feeInfo.platformNativeShare = 100
        await expect(feeManager.connect(wallet).setIntegratorFees(wallet.address,feeInfo)).to.emit(feeManager,"SetIntegratorFees");
