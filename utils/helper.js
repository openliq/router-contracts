...
exports.setRouters = async function(rubicAdapter_addr,network) {
    ...
    let symbiosisMetaRouter = symbiosis.config[network].metaRouter ? symbiosis.config[network].metaRouter : "0x0000000000000000000000000000000000000000"
    let symbiosisGateway = symbiosis.config[network].gateway ? symbiosis.config[network].gateway : "0x0000000000000000000000000000000000000000"

    let result = await (await adapter.setRouters(symbiosisMetaRouter,symbiosisGateway,stargateRouter,xRouter)).wait();
