...
contract RemoteSwapper is Ownable2Step {
    ...
    function _swapAndCall(bytes32 _transactionId,address _srcToken, uint256 _amount,bytes memory _swapData,bytes memory _callbackData,uint256 recoverGas) internal{
        Temp memory temp;
        ...
        if(_swapData.length > 0) {
            ...
            if(gasleft() > recoverGas){
                try this.doRemoteSwap{gas:gasleft() - recoverGas}(swap,_srcToken,temp.srcAmount) returns (address target,address dstToken,uint256 dstAmount) {
        ...
        if(_callbackData.length > 0 ){
            ...
            if(result && gasleft() > recoverGas){
                try this.doRemoteCall{gas:gasleft() - recoverGas}(callParam,temp.swapToken,temp.swapAmount) returns (address target,uint256 callAmount) {
                        temp.target = target;
