...
let {deployFeeManager,initialFeeStruct,setIntegratorFees,withdrawPlatformFees} = require("./utils/tronFeeManager.js")

task("feeManager:deploy", "deploy feeManager")
...
task("feeManager:initialFeeStruct", "initialFeeStruct feeManager")
    ...
    .setAction(async (taskArgs,hre) => {
        ...
        if(network.name === 'Tron' || network.name === 'TronTest'){
            await initialFeeStruct(hre.artifacts,network.name,taskArgs.feetype,taskArgs.fixedplatformnativefee,taskArgs.platformtokenfee);
        } else {
            console.log("deployer :", deployer);
            let deploy = await readFromFile(network.name);
            ...
            await (await feeManager.initialFeeStruct(taskArgs.feetype,taskArgs.fixedplatformnativefee,taskArgs.platformtokenfee)).wait()
        }
...
task("feeManager:setIntegratorFees", "setIntegratorFees feeManager")
    ...
    .setAction(async (taskArgs,hre) => {
        ...
        const { deployer } = await getNamedAccounts();
        if(network.name === 'Tron' || network.name === 'TronTest'){
            await setIntegratorFees(hre.artifacts,network.name,taskArgs.integrator,taskArgs.feetype,taskArgs.tokenfee,taskArgs.platformtokenshare,taskArgs.platformnativeshare,taskArgs.fixednativeamount);
        } else {
            console.log("deployer :", deployer);
            let deploy = await readFromFile(network.name);
            if(!deploy[network.name]["FeeManager"]){
