...
task("CCTPAdapter:deploy", "deploy CCTPAdapter").setAction(async (taskArgs, hre) => {
    ...
    if (cctp) {
        ...
        let param = ethers.utils.defaultAbiCoder.encode(
            ...
            [deployer, cctp.tokenMessenger, cctp.messageTransmitter]
        );
        ...
        const verifyArgs = [deployer, cctp.tokenMessenger, cctp.messageTransmitter]
        ...
        await verify(
            cctpAdapter,
            [deployer, cctp.tokenMessenger, cctp.messageTransmitter],
            "contracts/CCTPAdapter.sol:CCTPAdapter",
